...
| 데이터 포인트 | 최근 60개 |
| 갱신 주기 | 2초 |
| 표시 센서 | O₂ (좌축), H₂S, CO, VOC (우축) |
| Y축 | 좌축 15–23 (O₂), 우축 0–auto (가스류) |
...
| `/twin` | Twin | 3D 디지털 트윈 — Spark 2.0 가우시안 스플래팅 + 안전 데이터 오버레이 |
...
3D 씬이 전체 영역을 차지하고, 안전 데이터가 DOM 오버레이로 표시된다.

```
┌─────────────────────────────────────────────────────────┐
│ TopBar + Sidebar (공통 레이아웃)                          │
├─────────────────────────────────────────────────────────┤
│                                          ┌────────────┐ │
│  [센서 마커]     3D Gaussian Splat       │ Legend      │ │
│   O₂ 20.8%      (Three.js + Spark 2.0)  │ SAFE  ●    │ │
│   H₂S 1.2ppm                            │ CAUTION ●  │ │
│                                          │ WARNING ●  │ │
│  [작업자 마커]                            │ CRITICAL ● │ │
│   Worker A ● Active                      └────────────┘ │
...
| 요소 | 위치 | 업데이트 방식 |
|------|------|-------------|
| 센서 마커 | 3D 앵커 → 2D 투영 | `requestAnimationFrame` + `ref` 직접 DOM 조작 |
| 작업자 마커 | 3D 앵커 → 2D 투영 | `requestAnimationFrame` + `ref` 직접 DOM 조작 |
| 이벤트 토스트 | 우측 하단 고정 | React 상태 |
| 위험 등급 범례 | 우측 상단 고정 | 정적 |
| 구역 타이틀 | 좌측 상단 고정 | 매니페스트 title |

### 페이지 간 Twin 네비게이션

Dashboard, Zones, Events 페이지에서 구역별 "Open Twin" 버튼을 통해 해당 구역의 Twin 뷰로 이동할 수 있다.

| 출발 | 버튼 위치 | 동작 |
|------|----------|------|
| Dashboard → Twin | ZoneOverviewPanel 각 구역 행 | 구역 선택 + `/twin` 이동 |
| Zones → Twin | 각 구역 카드 하단 | 구역 선택 + `/twin` 이동 |
| Events → Twin | 각 이벤트 행 우측 아이콘 | 해당 구역 선택 + `/twin` 이동 |
